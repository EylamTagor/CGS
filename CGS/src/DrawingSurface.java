...
public class DrawingSurface extends PApplet {
	...
	private int status = 1;//1 instructinos 2 win 3 dnats 4 psnats 5 bbnats 6 snats 7 fail 8 natsconf 9 bb 10 space 11 driving 12 ps 13 nats 14 confscreen 15 conf1 16 conf2 17 conf3 18 conf4 19 conf5 20 playgame
	...
	public void drawInstructions() {
		...
		//if(status == 1) {
			if(slide == 1) {
				...
				textSize(20);
				fill(255,0,0);
				text("Choose the 5 topics you want to study(only then will the button appear)", 25, 25);
				options[0].draw(this);
				options[1].draw(this);
				options[2].draw(this);
				options[3].draw(this);
...
//		System.out.println(Arrays.toString(player.getProgress()));
	...
	public void drawBirdBlunderGame() {
		// TODO Auto-generated method stub
			...
			BirdBlunder drawing = new BirdBlunder(as.get(index),birdspersec, spaceConfIndicator, player, as, wronganswers, index);
			PApplet.runSketch(new String[]{""}, drawing);
			PSurfaceAWT surf = (PSurfaceAWT) drawing.getSurface();
			PSurfaceAWT.SmoothCanvas canvas = (PSurfaceAWT.SmoothCanvas) surf.getNative();
			JFrame window = (JFrame)canvas.getFrame();
	...
	public void drawPsychoSearchGame() {
		// TODO Auto-generated method stub
			...
			PsychoSearch drawing = new PsychoSearch(as.get(index),seconds, player, spaceConfIndicator, as, wronganswers, index);
			PApplet.runSketch(new String[]{""}, drawing);
			PSurfaceAWT surf = (PSurfaceAWT) drawing.getSurface();
			PSurfaceAWT.SmoothCanvas canvas = (PSurfaceAWT.SmoothCanvas) surf.getNative();
			JFrame window = (JFrame)canvas.getFrame();
	...
	public void mouseClicked() {
		...
		}else {//1 instructinos 2 win 3 dnats 4 psnats 5 bbnats 6 snats 7 fail 8 natsconf 9 bb 10 space 11 driving 12 ps 13 nats 14 confscreen 15 conf1 16 conf2 17 conf3 18 conf4 19 conf5 20 playgame}
			if(status == 20) {
